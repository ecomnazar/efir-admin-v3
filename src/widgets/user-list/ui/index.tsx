    ...
    <>
      <div className="w-full bg-secondary rounded-md pb-4">
        ...
        <ul>
          ...
          {users?.map((user) => {
            return (
              <li key={user.id} className="flex border-primary border-t border-b px-8 py-2 border-opacity-20 flex-wrap justify-between items-center">
                <div className="basis-[20%] text-[16px] flex items-center gap-x-3">
                    {user.avatar ? <img className="w-[45px] h-[45px] rounded-full object-fit object-cover" src={user.avatar} /> : <div className="w-[45px] h-[45px] rounded-full bg-slate-600"></div>}
                    <div>
                        <p className="text-[15px]">{user.username}</p>
                        <p className="text-[11px] text-textColor/50">{user.phone_number}</p>
                    </div>
                </div>
                <div className="basis-[20%] text-[15px]">{user.city.name}</div>
                <div className="basis-[20%] text-[15px]">Posts</div>
                <div className="basis-[20%] text-[16px]">
                    <div className={clsx("inline-block  text-[13px] rounded-md py-1 px-3", {
